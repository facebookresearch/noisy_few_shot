...
def gen_prototypes(embeddings, ways, shots, agg_method="mean"):
    assert embeddings.size(0) == ways * shots, "# of embeddings ({}) doesn't match ways ({}) and shots ({})".format(
    ...
    if agg_method == "mean":
        ...
        for i in range(5):
            ...
            dw = - torch.sum(errors / denom, axis=1, keepdims=True) / torch.sum(1.0 / denom, axis=1, keepdims=True)
            median_embeddings += dw
        return torch.squeeze(median_embeddings, dim=1)
    
    elif agg_method.startswith("cosine") or agg_method.startswith("euclidean") or agg_method.startswith("abs"):
        epsilon = 1e-6
        
        if agg_method.startswith("cosine"):
            # Normalize all embeddings to unit vectors
            norm_embeddings = embeddings/(torch.norm(embeddings, dim=2, keepdim=True) + epsilon)
            ...
            dist = ((embeddings.unsqueeze(dim=2)-embeddings.unsqueeze(dim=1))**2).sum(dim=-1)
            attn = -torch.sum(dist, dim=1) / (shots-1)
        elif agg_method.startswith("abs"):
            # dist: ways x shots x shots  
            dist = (torch.abs(embeddings.unsqueeze(dim=2)-embeddings.unsqueeze(dim=1))).sum(dim=-1)
            attn = -torch.sum(dist, dim=1) / (shots-1)
...
def gen_subset_prototypes(support_embeddings, shot, ways, subset_proportion=0.4, num_ensembles=10):
    ...
    for i in range(num_ensembles):
        subsample_indices_mask_i = np.concatenate([np.random.permutation(mask_array) for _ in range(ways)])
        sampled_support_embeddings = support_embeddings[subsample_indices_mask_i]
        support_prototypes_i = sampled_support_embeddings.reshape(ways, num_support_samples, -1).mean(dim=1)
...
def gen_hybrid_sample(x1, x2, alpha):
    return alpha*x1 + (1-alpha)*x2

def gen_hybrid_samples(support, ways, shot, alpha, beta):
    support_KN = support.reshape(ways, shot, -1)
    arng = torch.arange(shot)
    
    hybrids = []
    for support_i in support_KN:
        rolled_support = [support_i[np.roll(arng, i)] for i in range(1, beta+1)]
        hybrids.append(
            torch.vstack(
                [gen_hybrid_sample(support_i, rolled_support_i, alpha) for rolled_support_i in rolled_support]
            )
        )
    return torch.vstack(hybrids)

def soft_k_means(support, hybrids, centers):
    embeddings = torch.vstack([support, hybrids]).unsqueeze(dim=1)
    c = centers.unsqueeze(dim=0)
    
    dist = torch.sum((embeddings - c)**2, dim=-1)
    inv_weights = torch.sum(dist.unsqueeze(dim=2)/dist.unsqueeze(dim=1), dim=2)
    weights = 1/inv_weights
    
    new_centers = torch.sum(weights.unsqueeze(dim=2) * embeddings, dim=0) \
                    / torch.sum(weights, dim=0).unsqueeze(dim=1)
    return new_centers

def hard_k_means(support, hybrids, centers):
    embeddings = torch.vstack([support, hybrids]).unsqueeze(dim=1)  
    c = centers.unsqueeze(dim=0)
    
    dist = torch.sum((embeddings - c)**2, dim=2)
    closest = torch.argmin(dist, dim=1)
    
    new_centers = torch.vstack(
        [torch.mean(embeddings[closest==i], dim=0) for i in range(centers.shape[0])]
    )
    return new_centers
...
class MetaTrainer:
    ...
    def prepare_data(self, batch, ways, shot, query_num, support_label_noise_choices, outlier_batch=None):
        ...
        if support_label_noise > 0:
            data, labels, noise_positions = add_noise(
                data, labels, mask_indices, ways, support_label_noise, self.args.noise_type, outlier_data,
            )
    ...
    def proto_loss_acc(
        ...
        query_num = self.args.train_query if self.model.training else self.args.test_query
        ...
        support_labels = torch.repeat_interleave(torch.arange(ways), shot).to(self.device)
        ...
        if self.args.comp_method in ["proto", "rnnp_soft", "rnnp_hard"]:
            support_prototypes = outputs["support_prototypes"]
               
            # RNNP
            if "rnnp" in self.args.comp_method:
                hybrids = gen_hybrid_samples(
                    support_embeddings, ways, shot, self.args.rnnp_alpha, self.args.rnnp_beta
                )
                
                for _ in range(self.args.rnnp_iters):
                    if self.args.comp_method == "rnnp_soft":
                        support_prototypes = soft_k_means(support_embeddings, hybrids, support_prototypes)
                    elif self.args.comp_method == "rnnp_hard":
                        support_prototypes = hard_k_means(support_embeddings, hybrids, support_prototypes)
                    else:
                        raise NotImplementedError
            
            # Compare query embeddings with prototypes
            logits = self.metric(query_embeddings, support_prototypes)/self.args.logit_temperature
        ...
        # Match query embedding with support embeddings        
            ...
            one_hot_support_labels = torch.zeros(len(support_labels), ways).to(self.device)
            ...
            support_embeddings_unit = support_embeddings/torch.norm(support_embeddings, dim=1, keepdim=True)
            query_embeddings_unit = query_embeddings/torch.norm(query_embeddings, dim=1, keepdim=True)
            similarities = F.softmax(query_embeddings_unit @ support_embeddings_unit.T, dim=1)
        ...
        if self.args.binary_outlier_loss_weight > 0:
            noise_labels = torch.tensor(noise_positions[mask_indices["support"]]).unsqueeze(dim=1).to(self.device)
            accs["binary_outlier"] = accuracy(outputs["outlier_logits"], noise_labels, binary=True)
            losses["binary_outlier"] = self.args.binary_outlier_loss_weight * F.binary_cross_entropy_with_logits(
        ...
        if self.args.clean_proto_loss_weight > 0:
            clean_support_embeddings = support_embeddings[~noise_positions[mask_indices["support"]].astype(bool)]
            num_clean_shots = shot - int(noise_positions[mask_indices["support"]].sum()/ways)
            clean_protos = gen_prototypes(clean_support_embeddings, ways, num_clean_shots)
            
            # Distance of clean protos from predicted protos
            if self.args.dist_metric == "euclidean":
                losses["dist_to_clean_protos"] = self.args.clean_proto_loss_weight * (
                    (outputs["support_prototypes"] - clean_protos)**2
                ).sum() / ways
            elif self.args.dist_metric == "cosine":
                epsilon = 1e-6
                norm_protos = outputs["support_prototypes"]/(
                    torch.norm(outputs["support_prototypes"], dim=1, keepdim=True) + epsilon
                )
                norm_clean_protos = clean_protos/(torch.norm(clean_protos, dim=1, keepdim=True) + epsilon)
                
                # Negative, because we want to maximize cosine angle
                losses["dist_to_clean_protos"] = -self.args.clean_proto_loss_weight * (
                    norm_protos.flatten() * norm_clean_protos.flatten()
                ).sum() / ways
            else:
                raise NotImplementedError
    ...
    def nearest_neighbor(
        ...
        tallied = one_hot_labels.mean(dim=1) + torch.randn((ways*query_num, ways)).to(self.device)/1e4
