...
class MiniImagenet(data.Dataset):
    ...
    **Example**
    ...
    def __init__(
        self, root, mode="train", transform=None, target_transform=None, download=False
    ):
    ...
    def _check_exists(self):
        return os.path.exists(
            os.path.join(self.root, "mini-imagenet-cache-" + self.mode + ".pkl")
        )


class MiniImagenetOutlier(data.Dataset):
    def __init__(self, root, mode="train", transform=None):
        super(MiniImagenetOutlier, self).__init__()
        self.root = os.path.expanduser(root)
        self.transform = transform

        pickle_file = os.path.join(
            self.root, "mini-imagenet-outlier-cache-" + mode + ".pkl"
        )
        with open(pickle_file, "rb") as f:
            self.data = pickle.load(f)
