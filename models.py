...
class Convnet(nn.Module):
    ...
    def __init__(self, model="conv4", conv_proj_dim=None, res12_load_checkpoint_path=None):
        super().__init__()
        if model == "conv4":
            self.encoder = l2l.vision.models.ConvBase(output_size=64,
                                                      hidden=64,
                                                      channels=3,
                                                      max_pool=True)
            ...
            self.encoder = Res12(avg_pool=True, dropblock_size=5)
            if res12_load_checkpoint_path:
                pretrained_dict = torch.load(res12_load_checkpoint_path, map_location=torch.device('cpu'))['hyperformer']
                pretrained_dict = {k[13:]: v for k, v in pretrained_dict.items() if k[13:] in self.encoder.state_dict()}
                self.encoder.load_state_dict(pretrained_dict)
            
            self.conv_out_dims = 640
        else:
...
class Transformer(nn.Module):
    ...
    def __init__(
            self, ways, shot, num_layers, nhead, d_model, dim_feedforward, device, 
            cls_type="cls_learn", pos_type="pos_learn", agg_method="mean", transformer_metric="dot_prod",
        ):
        super().__init__()
        self.ways = ways
        self.shot = shot
        ...
        if self.cls_type == "cls_learn":
            self.cls_embeddings = nn.Embedding(max(ways["train"], ways["test"]), dim_feedforward)
        elif self.cls_type == "rand_const":
            self.cls_embeddings = nn.Embedding(max(ways["train"], ways["test"]), dim_feedforward).requires_grad_(False)
        
        if self.pos_type == "pos_learn":
            self.pos_embeddings = nn.Embedding(max(ways["train"], ways["test"]), dim_feedforward)
        elif self.pos_type == "rand_const":
            self.pos_embeddings = nn.Embedding(max(ways["train"], ways["test"]), dim_feedforward).requires_grad_(False)
        
        encoder_layer = TransformerEncoderLayer(
            d_model, nhead, dim_feedforward, dropout=0.1, activation='relu', metric=transformer_metric
        ) 
    ...
    def forward(self, x):
        ...
        cls_sup_embeds = torch.unsqueeze(cls_sup_embeds, dim=1)  # (ways*(shot+1), BS, dim)

        # Position embeddings based on class ID
        pos_idx = torch.cat((n_arng, torch.repeat_interleave(n_arng, shot)))
        pos_tokens = torch.unsqueeze(self.pos_embeddings(pos_idx), dim=1)  # (ways*(shot+1), BS, dim)
...
class ProtoConvnet(nn.Module):
    def __init__(
        self, ways, shot, model="conv4", conv_proj_dim=None, agg_method="mean", 
        binary_outlier_detection=False, res12_load_checkpoint_path=None
    ):
        super().__init__()
        
        self.convnet = Convnet(model, conv_proj_dim, res12_load_checkpoint_path)
        
        self.binary_outlier_detection = binary_outlier_detection
        if self.binary_outlier_detection:
            self.binary_outlier_detector = BinaryOutlierDetector(self.convnet.conv_out_dims)
    ...
    def forward(self, x, support_indices):
        ...
        outputs["support_prototypes"] = gen_prototypes(support_embeddings, ways, shot, self.agg_method)
...
class ProtoConvTransformer(nn.Module):
    ...
    def __init__(
        ...
        res12_load_checkpoint_path=None,
    ):
        super().__init__()
        
        self.convnet = Convnet(conv_model, conv_proj_dim, res12_load_checkpoint_path)
        if trans_d_model is None:
            trans_d_model = self.convnet.conv_out_dims
        self.transformer = Transformer(
            ways, shot, trans_layers, nhead, trans_d_model, trans_d_model, 
            device, cls_type, pos_type, transformer_metric=transformer_metric
        )
        
        self.ortho_proj = ortho_proj
        if self.ortho_proj:
            self.proj_trans_in = nn.Parameter(torch.nn.init.orthogonal_(torch.empty(self.convnet.conv_out_dims, trans_d_model)))
    ...
    def forward(self, x, support_indices):
        ...
        transformer_input = embeddings[support_indices] @ self.proj_trans_in if self.ortho_proj else embeddings[support_indices]
        transformer_output = self.transformer(transformer_input).squeeze(1)  # squeeze out batch size dim
        if self.ortho_proj:
            transformer_output = transformer_output @ self.proj_trans_out.T
            
            if self.ortho_proj_residual:
                conv_support_prototypes = gen_prototypes(embeddings[support_indices], ways, shot, self.agg_method)
                input_skip = torch.cat([conv_support_prototypes, embeddings[support_indices]], dim=0)
        ...
        if self.output_from_cls:
            ...
            outputs["support_prototypes"] = gen_prototypes(transformer_output[ways:], ways, shot, self.agg_method)
            
        if self.binary_outlier_detection:
            outputs["outlier_logits"] = self.binary_outlier_detector(transformer_output[ways:])
