...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="proto",
        help="Comparison method. Choices: proto, nearest_[k], match, lin_cls",
    )
    parser.add_argument(
        "--dist-metric",
        type=str,
    ...
    parser.add_argument(
        "--train-support-label-noise-choices", type=float, nargs="+", default=[0.0]
    )
    parser.add_argument("--train-query-label-noise", type=float, default=0.0)
    parser.add_argument(
        "--test-support-label-noise-list", type=float, nargs="+", default=[0.0]
    )
    ...
    if args.transformer_layers > 0:
        model = ProtoConvTransformer(
            ...
            trans_layers=args.transformer_layers,
            trans_d_model=args.trans_d_model,
            nhead=args.nhead,
            ortho_proj=args.ortho_proj,
            ortho_proj_residual=args.ortho_proj_residual,
            cls_type=args.cls_type,
            pos_type=args.pos_type,
            agg_method=args.agg_method,
            transformer_metric=args.transformer_metric,
            output_from_cls=args.output_from_cls,
    ...
    if args.warm_up_epochs > 0:
        lr_scheduler = GradualWarmupScheduler(
            optimizer,
            multiplier=1,
            total_epoch=args.warm_up_epochs,
    ...
    output_dirname = os.path.join(args.output_dir, args.name)
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for i in range(100):
            ...
            outlier_batch = (
                next(iter(data_loaders["outlier_train"]))
        ...
        for i, batch in enumerate(data_loaders["valid"]):
            outlier_batch = (
                next(iter(data_loaders["outlier_test"]))
                if args.noise_type == "outlier"
                else None
            )
        ...
        viz.line(
            ...
            Y=np.column_stack((train_acc, valid_acc)),
            win=acc_win,
            update="append",
        )
        viz.line(
            X=np.column_stack((epoch, epoch)),
            Y=np.column_stack((train_loss, valid_loss)),
            win=loss_win,
            update="append",
        )
        viz.line(
            X=np.array([epoch]),
            Y=np.array([lr_scheduler.get_last_lr()]),
            win=lr_win,
            update="append",
        )
        if args.embed_proto_loss_weight > 0:
            viz.line(
                ...
                Y=np.column_stack(
                    (
                        train_losses["conv_embed_proto"].detach().cpu(),
                        valid_losses["conv_embed_proto"].detach().cpu(),
                    )
                ),
                win=embed_proto_loss_win,
                update="append",
            )
        if args.binary_outlier_loss_weight > 0.0:
            viz.line(
                ...
                Y=np.column_stack(
                    (
                        train_losses["binary_outlier"].detach().cpu(),
                        valid_losses["binary_outlier"].detach().cpu(),
                    )
                ),
                win=outlier_loss_win,
                update="append",
            )
            viz.line(
                X=np.column_stack((epoch, epoch)),
                Y=np.column_stack(
                    (
                        train_accs["binary_outlier"].detach().cpu(),
                        valid_accs["binary_outlier"].detach().cpu(),
                    )
                ),
                win=outlier_acc_win,
                update="append",
        ...
        if args.clean_proto_loss_weight > 0.0:
            viz.line(
                ...
                Y=np.column_stack(
                    (
                        train_losses["dist_to_clean_protos"].detach().cpu(),
                        valid_losses["dist_to_clean_protos"].detach().cpu(),
                    )
                ),
                win=clean_proto_loss_win,
                update="append",
            )
